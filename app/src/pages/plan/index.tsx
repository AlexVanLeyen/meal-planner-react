...
const Plan: React.FC = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useMealPlanQuery(params.identifier ?? "");
  const { meals = [] } = plan ?? {};
  const [title, setTitle] = useState("");
  const debouncedTitle = useDebouncedValue(title);

  const onChangePlan = useCallback<(meal: MealModel) => void>(
    async (modifiedMeal) => {
      const modifiedPlan = { meals };
      const index =
        meals.findIndex(
          (meal) =>
            meal.date === modifiedMeal.date && meal.type === modifiedMeal.type,
        ) ?? -1;
  ...
  const onChangeTitle: React.ChangeEventHandler<HTMLInputElement> = useCallback(
    (e) => {
