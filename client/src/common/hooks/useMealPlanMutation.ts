...
const createPlan = async (
  plan: PlanModel,
): Promise<PlanModel & withIdentifier> => await Request.create(API_URL, plan);
...
const PlanAction: PlanAction = {
  PATCH: async (queryClient, { request, params }) => {
    ...
    if (request.method !== "PATCH" || !id || !formData.has("plan"))
      throw request;

    // there has to be a better way to handle this.
    const plan = JSON.parse(
      formData.get("plan") as string,
    ) as Partial<PlanModel>;
